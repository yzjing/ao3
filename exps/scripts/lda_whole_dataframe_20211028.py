...
def main(fandom):
    ...
    df_all['Dist'] = df_all['Processed_text'].map(lambda x: get_dist(x, model, id2word))


    print(fandom, ' ', len(df_all))
    df_all.to_csv(fandom + '_lda_with_dist_merged_chs_20210915_whole_dataframe.tsv', index = False, sep = '\t')
